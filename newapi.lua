...
API.stop_animation = function()
    ...
    -- Reset all joint Transforms back to identity so the Animator
    -- takes full control again (new API: don't touch C0 at all).
    for _, motor in pairs(mevreanim.animation.joints) do
    ...
    table.clear(mevreanim.animation.joints);
    mevreanim.animation.state = {
        is_playing   = false;
        current_url  = nil;
        speed        = 1.0;
        keyframes    = nil;
...
API.play_animation = function(url, speed)
    ...
    -- Stop any Animator-driven tracks and disable the Animate script
    -- so it doesn't fight our Transform writes.
    local clone_humanoid = clone:FindFirstChildOfClass("Humanoid") or clone:FindFirstChildOfClass("AnimationController");
    ...
    -- Find the Animator so we can respect EvaluationThrottled (LOD).
    ...
    -- Cache every Motor6D on the clone.
    ...
    -- Use PreSimulation (replaces RenderStepped for joint manipulation).
    -- We write motor.Transform instead of motor.C0 so we compose *on top of*
    -- whatever the Animator already placed, matching Roblox's new pipeline.
    mevreanim.connections.animation_hb = mevreanim.services.run_service.PreSimulation:Connect(function(deltaTime)
        if not anim.state.is_playing then return end;

        -- Respect LOD throttling: if the Animator skipped evaluation this
        -- frame, our pose data is stale so skip as well.
        ...
        if not current_frame then
            -- Wrap-around: between last and first frame.
        ...
        for partName, pose_cframe in pairs(current_frame.Data) do
            ...
            if motor and motor.Parent then
                ...
                local interpolated = next_pose_cframe
                    and pose_cframe:Lerp(next_pose_cframe, alpha)
                    or  pose_cframe;

                -- Compose our pose on top of the Animator's Transform.
                -- This matches the new Roblox animation pipeline.
                motor.Transform = interpolated * motor.Transform;
            end
        end
    end);
end;
